...
class UtilsTableview {
	...
	static COL_TRANSFORM_SOURCE = {name: "Source", transform: (it) => `<span class="${Parser.sourceJsonToColor(it)}" title="${Parser.sourceJsonToFull(it)}" ${Parser.sourceJsonToStyle(it.source)}>${Parser.sourceJsonToAbv(it)}</span>`};
	// endregion
}
