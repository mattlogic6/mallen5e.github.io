...
async function pTestDir ({errors, dir}) {
	...
	await pTestFile({errors, foundryData, foundryPath, originalDatas});
}
...
async function pTestRoot ({errors}) {
	...
	for (const foundryFile of foundryFiles) {
		...
		await pTestFile({errors, foundryData, foundryPath, originalDatas: [originalData]});
	}
}
...
async function pTestSpecialMagicItemVariants ({foundryData, originalDatas, errors}) {
	const variants = await pLoadData("magicvariants.json", `./data/magicvariants.json`);
	const prop = "variant";
	doCompareData({prop, foundryData, originalDatas: [variants], errors});
}

function doCompareData ({prop, foundryData, originalDatas, errors}) {
...
async function pTestFile ({foundryPath, foundryData, originalDatas, errors}) {
	Object.entries(foundryData)
		.forEach(([prop, arr]) => {
			...
			doCompareData({prop, foundryData, originalDatas, errors});
		});
}

const SPECIAL_PROPS = {
	"raceFeature": testSpecialRaceFeatures,
	"variant": pTestSpecialMagicItemVariants,
};
